...
const detectVerticalSquash = (img, iw, ih) => {
    let canvas = document.createElement('canvas');
    canvas.width = 1;
    canvas.height = ih;
    let ctx = canvas.getContext('2d');
...
const renderImageToDataURL = (img, options, doSquash) => {
    let canvas = document.createElement('canvas');
    renderImageToCanvas(img, canvas, options, doSquash);
    return canvas.toDataURL("image/jpeg", options.quality || 0.8);
};
const renderImageToCanvas = (img, canvas, options, doSquash) => {
    let iw = img.naturalWidth, ih = img.naturalHeight;
    if (!(iw+ih)) return;
...
const transformCoordinate = (canvas, ctx, width, height, orientation) => {
    switch (orientation) {
        case 5:
        case 6:
        case 7:
...
const MegaPixImage = function(srcImage) {
